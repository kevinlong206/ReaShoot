...
- `src/platform/ffmpeg/` - Shared FFmpeg recorded-file playback preview decoder/renderer used by macOS and Windows platform adapters.
...
macOS recorded-file playback preview requires FFmpeg headers/dylibs. Install with Homebrew or set `FFMPEG_ROOT` / `REASHOOT_FFMPEG_ROOT` to a prefix containing `include/` and `lib/`; the macOS build fails if FFmpeg is missing.
...
- Recorded-file playback preview uses shared FFmpeg code on macOS and Windows to decode preview frames for the SWELL panel. The small preview detail line shows whether live preview and recorded-file playback are using hardware or software decode and names the decode system. macOS live preview and FFmpeg playback use VideoToolbox when hardware decode is available; Windows currently reports its software live/playback decode paths until D3D11VA/DXVA is added. Audio is heard only through REAPER.
...
- Shared FFmpeg recorded-file playback preview (`src/platform/ffmpeg/ffmpeg_playback_preview.cpp`) decodes recorded `.mov` files for macOS and Windows. Recorded clips are heavily audio-interleaved (~3 audio packets per video packet), so `renderAt` must read a generous packet budget (200) to feed the decoder enough VIDEO packets. Keep the decoder on slice+frame threading (`FF_THREAD_FRAME | FF_THREAD_SLICE`, `thread_count=0`); frame threading is required for throughput so 4K decode keeps up with real time. Never re-seek/flush just because no frame has decoded yet (`soughtSinceOpen_` guard) — that flushed the frame-threading pipeline before it could prime and produced ZERO frames ("nothing plays"). Hardware acceleration is configured via `PlaybackOptions::hardwareDeviceType`; macOS uses `AV_HWDEVICE_TYPE_VIDEOTOOLBOX`.
...
- Recorded-file playback preview is decoded by the shared FFmpeg path. Avoid exact-seeking/flushing every timer tick; preserve the shared seek policy and decoder priming guard.
