...
This is a greenfield Swift project for controlling iPhone video recording from a Mac over local Wi-Fi.
...
3. iPhone records video with AVFoundation.
4. Mac sends stop, receives a recording descriptor, downloads the `.mov` over HTTP on port `8788`, verifies checksum, and acknowledges transfer.
...
- `Sources/video-sync-mac`: Mac command-line tool.
...
- Add an app UI control to reset pairing and copy/display connection details.
