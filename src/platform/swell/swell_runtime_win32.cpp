...
HWND makeButton(int isDefault, const char *label, int controlID, int x, int y, int width, int height, int flags) {
  ...
  return CreateWindowExA(0, "BUTTON", label ? label : "", style, x, y, width, height, g_currentParent,
                         reinterpret_cast<HMENU>(static_cast<intptr_t>(controlID)), GetModuleHandleA(nullptr), nullptr);
}
...
int comboAddString(HWND parent, int controlID, const char *text) {
  ...
  return combo ? static_cast<int>(SendMessageA(combo, CB_ADDSTRING, 0, reinterpret_cast<LPARAM>(text ? text : ""))) : -1;
}
...
void fillDialogBackground(HDC hdc, const RECT *rect, int level) {
  ...
  HBRUSH brush = GetSysColorBrush(level > 0 ? COLOR_BTNFACE : COLOR_WINDOW);
...
HFONT createFont(int height, int weight, const char *faceName) {
  return CreateFontA(height, 0, 0, 0, weight, FALSE, FALSE, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS,
                     CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, faceName ? faceName : "");
}
